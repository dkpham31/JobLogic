...
class ProductRepositoryImpl constructor(
    ...
    override suspend fun getBuyList(): Either<Failure, List<ItemToBuy>> = withHandlingException(ioDispatcher) {
            return@withHandlingException remoteDataSource.getBuyList().toProductEntities()
        }

    override suspend fun getSellList(): Either<Failure, List<ItemToBuy>> = withContext(ioDispatcher) {
            try {
