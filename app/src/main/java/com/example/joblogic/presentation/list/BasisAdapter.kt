...
import com.example.joblogic.presentation.list.holder.BasisViewHolder
import com.example.joblogic.presentation.list.holder.SellViewHolder
...
import com.example.joblogic.presentation.list.holder.CallViewHolder

class BasisAdapter: RecyclerView.Adapter<BasisViewHolder>() {

    private val items = mutableListOf<ItemData>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BasisViewHolder {
        ...
        return LayoutInflater.from(parent.context).inflate(viewType, parent, false).run {
            when (viewType) {
                ItemType.TYPE_CONTACT -> CallViewHolder(viewDataBinding as ItemContactBinding)
                ItemType.TYPE_PRODUCT -> SellViewHolder(viewDataBinding as ItemProductBinding)
                else -> throw UnsupportedOperationException("$viewType was not supported")
            }
        }
    }

    override fun onBindViewHolder(holder: BasisViewHolder, position: Int) {
